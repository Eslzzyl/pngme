...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, PngError> {
        ...
        let header: &[u8] = value[0..8].try_into().unwrap();
        if header != Self::STANDARD_HEADER {
            return Err(PngError::HeaderNotMatch);
        }
        let chunks = Self::chunks_from_bytes(&value[8..]).unwrap();
...
impl Display for Png {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        for i in Self::chunks(&self).iter() {
            write!(f, "Chunk {}: {}\n", curr_chunk, i)?;
            curr_chunk += 1;
        }
        Ok(())
...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk, PngError> {
        let index = self.chunks
            .iter()
            .position(
                |c| c.chunk_type().to_string() == chunk_type
            );
        match index {
            Some(i) => {
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        ...
        vec.append(&mut Self::STANDARD_HEADER.to_vec());
        for chunk in self.chunks.iter() {
            vec.append(&mut chunk.as_bytes().to_vec());
        }
        vec
    }

    fn chunks_from_bytes(bytes: &[u8]) -> Result<Vec<Chunk>, PngError> {        
        let mut pos: usize = 0;
        let mut chunks = Vec::<Chunk>::new();
        while pos < bytes.len() {
            let curr_chunk = Chunk::try_from(&bytes[pos..]).unwrap();
            pos += curr_chunk.as_bytes().len();
